...
const USER_AGENT = 'Mozilla/5.0 (compatible; SupergewinneCrawler/2.0)';
...
function crawlSupergewinne(PDO $pdo, array &$stats, array &$visitedListings, array &$visitedDetailUrls): void
{
    ...
    foreach ($entryPoints as $startUrl) {
        crawlListingWithPagination($startUrl, $visitedListings, function (DOMXPath $xpath, string $currentUrl) use ($pdo, &$stats, &$visitedDetailUrls): void {
            $detailLinks = [];
            $nodes = $xpath->query("//a[contains(translate(normalize-space(string()), 'ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÜ', 'abcdefghijklmnopqrstuvwxyzäöü'), 'mehr lesen')]");
            if ($nodes) {
...
function collectSupergewinneEntryPoints(): array
{
    ...
    $xpath = fetchHtml('https://www.supergewinne.de/');
    if ($xpath) {
        $navLinks = $xpath->query("//nav//a[contains(@href, '/gewinnspiele')]");
        if ($navLinks) {
            foreach ($navLinks as $node) {
                $href = trim($node->getAttribute('href'));
                if ($href === '') {
                    continue;
                }
                $absolute = makeAbsoluteUrl('https://www.supergewinne.de/', $href);
...
function findNextPageUrl(DOMXPath $xpath, string $currentUrl): ?string
{
    $expressions = [
        ...
        "//a[contains(@class,'page-numbers') and (contains(translate(normalize-space(string()), 'ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÜ', 'abcdefghijklmnopqrstuvwxyzäöü'), 'nächste') or contains(@class,'next'))]",
    ];
...
{
    $nodes = $xpath->query("//a[contains(translate(normalize-space(string()), 'ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÜ', 'abcdefghijklmnopqrstuvwxyzäöü'), 'jetzt direkt mitmachen') or contains(translate(normalize-space(string()), 'ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÜ', 'abcdefghijklmnopqrstuvwxyzäöü'), 'zum gewinnspiel') or contains(@class,'btn')]");
    if (!$nodes || !$nodes->length) {
        return null;
    }
    foreach ($nodes as $node) {
